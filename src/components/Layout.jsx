...
// Default avatar path (place the file at src/assets/default-avatar.svg)
...
export default function Layout() {
  ...
  // Load profile on mount to display in header
  useEffect(() => {
    let mounted = true;
    async function load() {
      try {
        ...
        // if 401 occurs the interceptor will redirect; otherwise ignore
  ...
  return (
    <div className="flex min-h-screen w-full">
      ...
      <motion.aside
        ...
        className="sidebar flex flex-col h-screen sticky top-0 transition-all"
        style={{ background: "var(--sidebar-bg, #1e293b)", color: "var(--text-sidebar, #e6eef8)" }}
      >
        <div className="flex items-center justify-between p-4 border-b" style={{ borderColor: "rgba(255,255,255,0.04)" }}>
          {!collapsed && <h1 className="text-xl font-bold" style={{ color: "var(--text-sidebar, #e6eef8)" }}>BrandInsight</h1>}
          <button
            ...
            return (
              <Link key={item.name} to={item.path}>
                <motion.div
                  ...
                  className={`flex items-center gap-3 p-3 mx-3 rounded-xl cursor-pointer transition-all ${active ? "shadow" : ""}`}
                  style={{
                    color: active ? "white" : "var(--text-sidebar, #e6eef8)",
      ...
      <div className="flex-1 flex flex-col">
        {/* Header flush with sidebar */}
        <header
          className="header flex items-center justify-end p-3"
          style={{ background: "var(--header-bg, var(--sidebar-bg, #1e293b))", color: "var(--text-sidebar, #e6eef8)" }}
        >
